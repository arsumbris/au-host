...
// A REACT pane-projection: the live `--au-*` token editor over `<au-*>` custom elements. It externalizes
// the host's shared deps (React + the SDKs) so the dist emits bare imports that resolve at runtime to the
// one served au-shared:// instance via the host's injected import map.
export default defineConfig({
  plugins: [
    react(),
    // Drop the orphan CSS sidecar a Vite lib build re-emits (the host loader has no CSS handling; the
    // <au-*> components self-style in shadow and this pane's own layout CSS ships via host.styles.inject).
    {
